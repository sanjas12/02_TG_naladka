...
class FileHandler:
    ...
    def _detect_file_params(self, sample: bytes, second_row: bytes) -> None:
        ...
        self.model.delimiter = ";" if sample_str.count(";") > sample_str.count("\t") else "\t"
        self.model.decimal = "." if second_row_str.count(".") > second_row_str.count(",") else ","
...
class SignalManager:
    ...
    def _process_headers(self, df: pd.DataFrame) -> None:
        ...
        if self.model.is_time and self.model.is_ms:
            self.model.dict_all_signals[cfg.COMMON_TIME] = len(self.model.dict_all_signals) + 1
...
class PlotManager:
    ...
    def _get_signals_from_table(self, table: QTableWidget, signals_dict: Dict[str, int]) -> List[str]:
...
class MainLogic:
    ...
    def _setup_connections(self) -> None:
        ...
        for group_box, dict_signal in zip(
            ...
            group_box.btn_first.clicked.connect(lambda _, gb=group_box, ds=dict_signal: self.add_signal(gb, ds))
            group_box.btn_second.clicked.connect(lambda _, gb=group_box, ds=dict_signal: self.remove_signal(gb, ds))
    ...
    def _setup_time_axis(self) -> None:
        ...
        if self.model.time_signal not in self.model.dict_all_signals.keys():
            self._show_error(f"Сигнал времени '{self.model.time_signal}' не найден в данных")
            return
...
def test_FileHandker():
    ...
    test_gz_file = "tg-naladka\src\DATA_in\sarz\смол\step\ШУР11-2025-07-11_115842_1.csv.gz"
