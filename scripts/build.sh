...
# src/_revision.py, полностью пересоздаёт каталог build/, запускает сборку
# интерпретатором из .venv и создаёт сжатый ZIP рядом с standalone-папкой.
# В логе выводятся размеры сборки и самых крупных файлов. По умолчанию зависимости
# не изменяются и ничего не скачивается. Временный файл ревизии удаляется и после
# успешной сборки, и при ошибке.
...
find "$BUILD_DIR" -maxdepth 1 -type f -name "TG-Naladka-*.zip" -print
