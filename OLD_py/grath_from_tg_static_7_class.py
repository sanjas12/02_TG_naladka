...
class TG_naladka():
    ...
    # "Сравнение энкод/ОЗ одной стороны"
    def eq_enc(self):
        ...
        def reading_data(namedirection):
            ...
            return data_out, temp_time_2, name_TG, time_read
        ...
        def draw(name, name_column,  OZ_1, OZ_2):
            ...
            point_f = 10                                        #
                                    # x                   y
            line, =  ax.plot(OZ_1[1][1::point_f], OZ_1[0][name_column][1::point_f], lw=0.5, color='b', label="ОЗ_1_А")  #
...
if __name__ == '__main__':
    ...
    butSTEP.bind('<ButtonRelease-1>', TG_naladka.grath_stepper)

    # кнопка СКОРОСТИ
    butVel = Button(win_1, text="Скорости", width=30)
    butVel.grid(in_=frame_b, sticky=S, row=0, column=1)
    butVel.bind('<ButtonRelease-1>', TG_naladka.grath_velosity)

    # кнопка сравнение энкодеров по одной стороне
    but_Enc = Button(win_1, text="Сравнение энкод\n одной стороны")
    but_Enc.grid(in_=frame_b, sticky=S, row=1, column=0)
    but_Enc.bind('<ButtonRelease-1>', TG_naladka.eq_enc)

    # кнопка вывод показаний энкодрера сторона А.
    but_one_channel = Button(win_1, text="Показания ОЗ по одному каналу")
    but_one_channel.grid(in_=frame_b, sticky=S, row=1, column=1)
    but_one_channel.bind('<ButtonRelease-1>', TG_naladka.one_channel)
    ...
    but_ustavki.bind('<ButtonRelease-1>', TG_naladka.shift_oz)
