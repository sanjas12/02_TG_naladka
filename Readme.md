...
Поддерживает форматы CSV, GZ, TXT и LOG, включая LOG-архивы с колонкой
`timestamp` и разделителем-запятой. Переполнение 16-битного счётчика `timestamp`
автоматически разворачивается в непрерывную временную шкалу.
...
Готовая standalone-папка создаётся в `build/`; ZIP автоматически не создаётся.
В конце сборки выводятся её размер и список самых крупных файлов дистрибутива.
